...
&НаКлиенте
Процедура Декорация_ПолучитьТокенНажатие()

	IDПриложения = ВК_ИнтеграцияВызовСервера.ПолучитьIDПриложения();

	Если ЗначениеЗаполнено(IDПриложения) И ТипЗнч(IDПриложения) = Тип("Строка") Тогда
...
//  Строка, Неопределено - Команда запуска браузера
...
Процедура ОбновитьИнтерфейсИнтерактивногоОкна() Экспорт
	...
	Если ПараметрыПодключения.АвторизацияВыполнена Тогда
		
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Да'"));
